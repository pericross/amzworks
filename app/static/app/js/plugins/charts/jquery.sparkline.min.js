...
(function ($) {
    ...
    $.fn.sparkline.discrete = function (values, options, width, height) {
        ...
        if (target) {
            var canvas_height = target.pixel_height,
                ...
                pheight = canvas_height - line_height, min = Math.min.apply(Math, values),
    ...
    $.fn.sparkline.bullet = function (values, options, width, height) {
        ...
        if (target && values.length > 1) {
            var canvas_width = target.pixel_width - Math.ceil(options.get('targetWidth') / 2),
                canvas_height = target.pixel_height, min = Math.min.apply(Math, values),
                max = Math.max.apply(Math, values);
            ...
            var targetval = values[0],
                ...
                targettop = Math.round(canvas_height * 0.10), targetheight = canvas_height - (targettop * 2);
    ...
    $.fn.sparkline.pie = function (values, options, width, height) {
        ...
        if (target && values.length > 1) {
            var canvas_width = target.pixel_width, canvas_height = target.pixel_height,
                radius = Math.floor(Math.min(canvas_width, canvas_height) / 2), total = 0, next = 0,
                circle = 2 * Math.PI;
            for (var i = values.length; i--;) {
                total += values[i];
            }
    ...
    $.fn.sparkline.box = function (values, options, width, height) {
        ...
        var minvalue = options.get('chartRangeMin') === undefined ? Math.min.apply(Math, values) : options.get('chartRangeMin'),
            ...
            target = $(this).simpledraw(width, height, options.get('composite')), vlen = values.length, lwhisker,
            loutlier, q1, q2, q3, rwhisker, routlier;
        if (target && values.length > 1) {
            var canvas_width = target.pixel_width, canvas_height = target.pixel_height;
            if (options.get('raw')) {
